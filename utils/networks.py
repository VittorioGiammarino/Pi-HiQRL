...
class GCIQEValue(nn.Module):
    ...
    def __call__(self, 
        ...
        if is_phi:
            ...
            if self.encoder is not None and not state_encoded:
        ...
        sxy = jnp.take_along_axis(xy, ixy, axis=-1) #same as sxy=xy.sort(axis=-1)
...
class GCLinfValue(nn.Module):
    """GCLinf value function.

    This module computes the value function as an Linf-based metric.
    ...
    def __call__(self, observations, goals, is_phi=False, info=False):
        """Return the Linf value function.
        ...
        abs = jnp.sqrt((x-y)**2+1e-6) #abs function soft 
        abs_reshaped = jnp.reshape(abs, (abs.shape[0],-1,16)) # reshape
        softmax = (jsp.logsumexp(10*abs_reshaped, 2)-jnp.log(16))/10 #softmaximum 
        v = 0.2*(jnp.sum(softmax, axis=1, keepdims=True)) #external sum  
...
class PotentialFunction(nn.Module):
    ...
    def setup(self):
        self.potential_net = MLP((*self.hidden_dims, 1), activate_final=False, layer_norm=self.layer_norm)
    ...
    def __call__(self, goals):
        ...
        Args:
            goals: Goals.
        """
        c = self.potential_net(goals).squeeze(-1)
        return c
