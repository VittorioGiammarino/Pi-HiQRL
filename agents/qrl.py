...
from utils.networks import MLP, GCActor, GCDiscreteActor, GCIQEValue, GCMRNValue, LogParam, GCLinfValue
...
class QRLAgent(flax.struct.PyTreeNode):
    ...
    def create(
        ...
        elif config['quasimetric_type'] == 'linf':
            value_def = GCLinfValue(
                hidden_dims = config['value_hidden_dims'],
                latent_dim = config['latent_dim'],
                dim_per_component=8,
                layer_norm = config['layer_norm'],
