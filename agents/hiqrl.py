...
class HIQRL_Agent(flax.struct.PyTreeNode):
    """Hierarchical Quasimetric RL (HiQRL) agent.
    ...
    def process_batch_high_value_loss(self, batch):
        """Process the batch for the high-level state space abstraction"""
    ...
    def high_value_loss(self, batch, grad_params):
        ...
        if self.config['pi_high']:
            ...
            eikonal_loss = (jax.nn.relu(grad_norm_d - self.config['eikonal_speed_value_high']) ** 2).mean() 
            metrics['Eikonal_loss'] = eikonal_loss
            total_loss += eikonal_loss

        else:
            d_pos = self.network.select('high_value')(batch_observations, batch_next_observations, params=grad_params)
            lam = self.network.select('lam')(params=grad_params)
            d_pos_loss = (jax.nn.relu(d_pos - 1) ** 2).mean()
            metrics['d_pos_loss'] = d_pos_loss

            total_loss += d_pos_loss * jax.lax.stop_gradient(lam)
            total_loss += lam * (self.config['eps'] - jax.lax.stop_gradient(d_pos_loss))
    ...
    def low_value_loss(self, batch, grad_params):
        ...
        eikonal_loss = jnp.mean((grad_norm_d1 - self.config['eikonal_speed_value_low']) ** 2) + jnp.mean((grad_norm_d2 - self.config['eikonal_speed_value_low']) ** 2) 
...
def get_config():
    config = ml_collections.ConfigDict(
        dict(
            ...
            pi_high = True,
            pi_low = True,
            eikonal_speed_value_high = 1,
            eikonal_speed_value_low = 1,
